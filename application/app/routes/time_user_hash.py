...
game_class = Page("More Random Text", default_hint=default_hint)

game_class.load_scripts({"connection": "game_4.js"})

async def instructions(request: Request):
    text = ("You go to make a post on the social media site from the last game and you think there may be an "
                    "exploit here too after seeing how silly the last one was! There is a call to "
                    "get_authenticated_user which returns you similar "
                    "data to the last game, as well as your frontend that you can post to.")

    subtext = ("Hint: You cannot clone the outputs "
    ...
    secondary_button = generate_button("Get Authenticated User", f"{config.URL}{game_class.url_prefix}/get_authenticated_user", primary=False)
...
def valid_hash(data):
    ...
    if 'time' in data and 'userID' in data and 'imagePath' in data and 'hash' in data:
        try:
        ...
        if int(time.time()) - num_time < 30000:
            buffer = (data['time'] + data['userID']).encode('utf-8')
            hashed_time = sha256(buffer).hexdigest()
            if hashed_time == data['hash']:
                return "user"
            else:
                admin_buffer = (data['time'] + data['userID'] + "anlco8e7hc82q7c98027345oihbas0d897").encode('utf-8')
                admin_hashed_time = sha256(admin_buffer).hexdigest()
                if admin_hashed_time == data['hash']:
                    return "admin"

    return False


@game.get('/get_authenticated_user', response_class=JSONResponse)
async def get_connection_data():
    curr_time = int(time.time())
    user_id = 'SuperSafeUser'
    hashed_time = (str(curr_time) + user_id + "anlco8e7hc82q7c98027345oihbas0d897").encode('utf-8')
    payload = {
        'time': str(curr_time),
