...
class User(Document):
    ...
    async def upsert_hint(cls, cookie: str, level: str, hint_text: str):
        ...
        if level_obj is None:
            level_obj = user_obj.level_data[level] = await cls.create_level(level)
        ...
        else: raise Exception("No more than 3 hints can be requested")
    ...
    @classmethod
    async def insert_users(cls, data):
        existing = await User.find(
            In(User.user_access_code, ([user for user in data]))).to_list()

        existing_codes = {doc.user_access_code for doc in existing}

        # Filter to only new documents
        new_docs = [
            User(user_cookie=str(uuid4()), user_access_code=user)
            for user in data
            if user not in existing_codes
        ]

        if new_docs:
            await User.insert_many(new_docs)
    ...
    async def get_hint_length(self, level) -> int:
        level_obj = self.level_data.get(level, None)
        if level_obj is None:
            self.level_data[level] = await self.create_level(level)
            return 0
    ...
    async def complete_level(self, level: str):
        ...
        if level_data is None:
            level_data = self.level_data[level] = await self.create_level(level)
    ...
    async def get_last_hint(self, level: str) -> str:
        ...
        if level_obj is None:
            self.level_data[level] = await self.create_level(level)
            raise Exception("List cannot be greater than 2, and 0")
